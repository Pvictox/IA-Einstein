...
def calcularFitness(cromossomo):
   ...
   # 3) O sueco tem cachorros como animais de estimação
    ...
    for num_Cor in cromossomo.getCor():        
        if (enumCor(num_Cor).name == "VERDE"):
            ...
            if ( indexDaCasa != 4): #Existe casa a direita
                if (enumCor(cromossomo.getCor()[indexDaCasa+1]).name == "BRANCA"):
                        cromossomo.addPonto()
    
     # 6) O homem que vive na casa VERDE bebe CAFE
    for numCor in cromossomo.getCor():
        if (enumCor(numCor).name == "VERDE"):
    ...
    for numCigarro in cromossomo.getCigarro():
        if (enumCigarro(numCigarro).name == "BLENDS"):
            indexDaCasa = cromossomo.getCigarro().index(numCigarro)
            if (indexDaCasa == 0):
    ...
    for numNaciona in cromossomo.getNacionalidade():
        if (enumNac(numNaciona).name == "NORUEGUES"):
            indexDaCasa = cromossomo.getNacionalidade().index(numNaciona)
            if (indexDaCasa == 0):
    ...
    for numCigaro in cromossomo.getCigarro():
        if (enumCigarro(numCigaro).name == "BLENDS"):
            indexDaCasa = cromossomo.getCigarro().index(numCigaro)
            if (indexDaCasa == 0):
